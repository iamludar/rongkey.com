...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (isSelectAttr(spec_arr)) {
		...
		//Ajax.call('http://39.108.6.181/0rongkey/js/flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
        Ajax.call('http://39.108.6.181/0rongkey/js/flow.php?step=add_to_cart', 'goods=' + JSON.stringify(goods), addToCartResponse, 'POST', 'JSON');
	}
...
function addToCartNums(goodsId, parentId) {
	...
	for (i = 0; i < goodsIds.length; i++) {
		...
		if (i == buynum) {
			...
			Ajax.call('http://39.108.6.181/0rongkey/js/flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), '', 'POST', 'JSON');
		}
	...
	if (result.error > 0) {
		...
		// 没选规格，弹出属性选择框
			...
			/*if (confirm(result.message)) {
				location.href = 'http://39.108.6.181/0rongkey/js/user.php';
			}*/
		...
		var cart_url = 'http://39.108.6.181/0rongkey/js/flow.php?step=cart';
		if (cartInfo) {
...
function collect(goodsId) {
	Ajax.call('http://39.108.6.181/0rongkey/js/user.php?act=collect', 'id=' + goodsId, collectResponse, 'GET', 'JSON');
}
...
function question_type_curr(page, id, question_type) {
	...
	Ajax.call('http://39.108.6.181/0rongkey/js/question.php?act=gotopage', 'page=' + page + '&id=' + id + '&question_type=' + question_type, gotoPageResponse_question, 'GET', 'JSON');
}
...
function comment_type_curr(page, id, type, comment_level) {
	...
	Ajax.call('http://39.108.6.181/0rongkey/js/comment.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type + '&comment_level=' + comment_level, gotoPageResponse, 'GET', 'JSON');
}
...
function gotoBuyPage(page, id) {
	Ajax.call('http://39.108.6.181/0rongkey/js/goods.php?act=gotopage', 'page=' + page + '&id=' + id, gotoBuyPageResponse, 'GET', 'JSON');
}
...
function sendHashMail() {
	Ajax.call('http://39.108.6.181/0rongkey/js/user.php?act=send_hash_mail', '', sendHashMailResponse, 'GET', 'JSON')
}
...
function display_mode_wholesale(str) {
	...
	setTimeout(doSubmit, 0);
	function doSubmit() {
		document.forms['wholesale_goods'].action = "http://39.108.6.181/0rongkey/js/wholesale.php";
...
function addPackageToCart(packageId, indexId) {
	...
	Ajax.call('http://39.108.6.181/0rongkey/js/flow.php?step=add_package_to_cart', 'package_info=' + $.toJSON(package_info), addPackageToCartResponse, 'POST', 'JSON');
}
...
function addPackageToCartResponse(result) {
	if (result.error > 0) {
		if (result.error == 2) {
			if (confirm(result.message)) {
				location.href = 'http://39.108.6.181/0rongkey/js/user.php?act=add_booking&id=' + result.goods_id;
			}
		} else {
			alert(result.message);
		}
...
function openSpeDiv(message, goods_id, parent) {
	...
	newMask.style.opacity = "http://39.108.6.181/0rongkey/js/0.15";
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	html += "<div class=cartbntfloat ><a href='http://39.108.6.181/0rongkey/js/flow.php'><img src='jsico1-3.gif'/*tpa=http://39.108.6.181/0rongkey/js/themes/68ecshopcom_360buy/images/jsico1.gif*/></a><a href=\'javascript:cancel_div()\'><img src='goon_ico1-3.gif'/*tpa=http://39.108.6.181/0rongkey/js/themes/68ecshopcom_360buy/images/goon_ico1.gif*/></a></div>";
	html += '</div></div>';
	...
	newMask.style.opacity = "http://39.108.6.181/0rongkey/js/0.40";
	document.body.appendChild(newMask);
...
function chat_online(data){
	...
	$.post('http://39.108.6.181/0rongkey/js/chat.php', {act: 'check_login'}, function(result){
		if(result == 'false'){
			window.location.href = "http://39.108.6.181/0rongkey/js/chat.php";
		}else{
			var param = "";
