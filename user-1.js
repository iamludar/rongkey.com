...
function check_username(val) {
    ...
    /* 代码增加 By  www.68ecshop.com End */
		...
		Ajax.call('http://www.rongkey.com/english/js/user.php?act=check_username', 'username=' + val, checkusername_callback, 'GET', 'TEXT', true, true);
	}
}
...
function user_login(back_history){
	...
	if(back_history){
		Ajax.call( 'http://www.rongkey.com/english/js/user.php?act=act_login', 'username=' + username.val()+'&password='+password.val()+'&captcha='+captcha.val()+'&back_act='+back_act, return_login_back , 'POST', 'JSON');	
	}else{
		Ajax.call( 'http://www.rongkey.com/english/js/user.php?act=act_login', 'username=' + username.val()+'&password='+password.val()+'&captcha='+captcha.val()+'&back_act='+back_act, return_login , 'POST', 'JSON');
	}
return false;
}
...
function is_registered(username) {
	...
	Ajax.call('http://www.rongkey.com/english/js/user.php?act=is_registered', 'username=' + username, registed_callback, 'GET', 'TEXT', true, true);
}
...
function checkEmail(email, callback) {
	...
	// Ajax.call('http://www.rongkey.com/english/js/user.php?act=check_email', 'email=' + email,
...
function checkEmailExist(email, callback) {
	...
	// Ajax.call('http://www.rongkey.com/english/js/user.php?act=check_email', 'email=' + email,
	// check_email_callback, 'GET', 'TEXT', true, true);
	$.post('http://www.rongkey.com/english/js/user.php?act=check_email', {
		email: email
	}, function(result) {
		if (result == 'ok') {
...
function checkMobilePhone(mobile, callback) {
	...
	// Ajax.call('http://www.rongkey.com/english/js/user.php?act=check_mobile_phone', 'mobile_phone=' + mobileObj,
...
function checkMobilePhoneExist(mobile, callback) {
	...
	$.post('http://www.rongkey.com/english/js/user.php?act=check_mobile_phone', {
		mobile_phone: mobile
	}, function(result) {
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	checkEmail(emailObj, function(result) {
		if (result) {
			...
			var url = 'http://www.rongkey.com/english/js/user.php?act=send_email_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
				email: emailObj.val()
			}, function(result) {
				if (result == 'ok') {
...
function sendMobileCode(mobileObj, mobileCodeObj, sendButton) {
	...
	checkMobilePhone(mobileObj, function(result) {
		if (result) {
			// 发送邮件
			var url = 'http://www.rongkey.com/english/js/user.php?act=send_mobile_code&XDEBUG_SESSION_START=ECLIPSE_DBGP';
			$.post(url, {
...
function returnToCart(orderId) {
	Ajax.call('http://www.rongkey.com/english/js/user.php?act=return_to_cart', 'order_id=' + orderId, returnToCartResponse, 'POST', 'JSON');
}
