...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	if (isSelectAttr(spec_arr)) {
		...
		//Ajax.call('http://main.fuxiark.com/0rongkey/English/js/flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
        Ajax.call('http://main.fuxiark.com/0rongkey/English/js/flow.php?step=add_to_cart', 'goods=' + JSON.stringify(goods), addToCartResponse, 'POST', 'JSON');
	}
...
function addToCartNums(goodsId, parentId) {
	...
	for (i = 0; i < goodsIds.length; i++) {
		...
		if (i == buynum) {
			...
			Ajax.call('http://main.fuxiark.com/0rongkey/English/js/flow.php?step=add_to_cart', 'goods=' + $.toJSON(goods), '', 'POST', 'JSON');
		}
	...
	if (result.error > 0) {
		...
		// 没选规格，弹出属性选择框
			...
			/*if (confirm(result.message)) {
				location.href = 'http://main.fuxiark.com/0rongkey/English/js/user.php';
			}*/
		...
		var cart_url = 'http://main.fuxiark.com/0rongkey/English/js/flow.php?step=cart';
		if (cartInfo) {
...
function collect(goodsId) {
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/user.php?act=collect', 'id=' + goodsId, collectResponse, 'GET', 'JSON');
}
...
function question_type_curr(page, id, question_type) {
	...
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/question.php?act=gotopage', 'page=' + page + '&id=' + id + '&question_type=' + question_type, gotoPageResponse_question, 'GET', 'JSON');
}
...
function comment_type_curr(page, id, type, comment_level) {
	...
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/comment.php?act=gotopage', 'page=' + page + '&id=' + id + '&type=' + type + '&comment_level=' + comment_level, gotoPageResponse, 'GET', 'JSON');
}
...
function gotoBuyPage(page, id) {
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/goods.php?act=gotopage', 'page=' + page + '&id=' + id, gotoBuyPageResponse, 'GET', 'JSON');
}
...
function sendHashMail() {
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/user.php?act=send_hash_mail', '', sendHashMailResponse, 'GET', 'JSON')
}
...
function display_mode_wholesale(str) {
	...
	setTimeout(doSubmit, 0);
	function doSubmit() {
		document.forms['wholesale_goods'].action = "http://main.fuxiark.com/0rongkey/English/js/wholesale.php";
...
function addPackageToCart(packageId, indexId) {
	...
	Ajax.call('http://main.fuxiark.com/0rongkey/English/js/flow.php?step=add_package_to_cart', 'package_info=' + $.toJSON(package_info), addPackageToCartResponse, 'POST', 'JSON');
}
...
function addPackageToCartResponse(result) {
	if (result.error > 0) {
		if (result.error == 2) {
			if (confirm(result.message)) {
				location.href = 'http://main.fuxiark.com/0rongkey/English/js/user.php?act=add_booking&id=' + result.goods_id;
			}
		} else {
			alert(result.message);
		}
...
function openSpeDiv(message, goods_id, parent) {
	...
	newMask.style.opacity = "http://main.fuxiark.com/0rongkey/English/js/0.15";
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	// 生成层内内容
	html = '<div class=cardivfloat><span class=cartdivfloattitle>商品已成功添加到购物车！</span><a href=\'javascript:cancel_div()\' style="float:right;padding:0 26px 0 0;background:url(themes/68ecshopcom_360buy/images/ico_closebig1.gif) right center no-repeat;cursor:pointer;color:#ffffff;font-size:12px;" >关闭</a></div><div class="cartpopDiv"><div class="toptitle"><a href="http://main.fuxiark.com/0rongkey/English/js/goods.php?id=' + goods_id + '" class="pic"><img src=' + pic + ' width="98" height="98" style="border:#dddddd 1px solid; display:block;"/></a><p><font style="font-weight:bold">' + name + '</font>  <br>' + goods_brief + '<br>购买价格：<font style="color:#cc0000">' + price + '</font><br></p></div>';
	...
	html += "<div class=cartbntfloat ><a href='http://main.fuxiark.com/0rongkey/English/js/flow.php'><img src='jsico1-2.gif'/*tpa=http://main.fuxiark.com/0rongkey/English/js/themes/68ecshopcom_360buy/images/jsico1.gif*/></a><a href=\'javascript:cancel_div()\'><img src='goon_ico1-2.gif'/*tpa=http://main.fuxiark.com/0rongkey/English/js/themes/68ecshopcom_360buy/images/goon_ico1.gif*/></a></div>";
	html += '</div></div>';
	...
	newMask.style.opacity = "http://main.fuxiark.com/0rongkey/English/js/0.40";
	document.body.appendChild(newMask);
...
function chat_online(data){
	...
	$.post('http://main.fuxiark.com/0rongkey/English/js/chat.php', {act: 'check_login'}, function(result){
		if(result == 'false'){
			window.location.href = "http://main.fuxiark.com/0rongkey/English/js/chat.php";
		}else{
			var param = "";
