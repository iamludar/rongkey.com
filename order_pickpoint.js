function show(o,sid){ 
	...
	var pid = document.getElementById('point'+sid).value;
	 Ajax.call('http://www.rongkey.com/js/flow.php?step=select_pickinfo', 'sid=' + sid + '&pid='+pid, pickinfo, 'POST', 'JSON');
	o.style.display = "";
	$('#bg').show();
}
function hide(o){ 
...
function save_point(suppid){
	var value = $('#s_pid').val();
	Ajax.call('http://www.rongkey.com/js/flow.php?step=save_point', 'sid=' + suppid + '&pid='+value, selectpickinfo, 'GET', 'JSON');
	hide('pop');
}
