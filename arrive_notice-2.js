...
function tell_me(goods_id, no_have_val){
	...
	Ajax.call('http://39.108.6.181/0rongkey/themes/68ecshopcom_360buy/js/user.php?act=book_goods', 'id=' + goods_id + '&no_have=' + no_have, tellmeResponse, 'GET', 'JSON');
}
...
function tell_me1(goods_id, no_have_val){
	...
	if (msg){
		...
		no_have = (typeof(no_have_val) == "undefined" ? 0 : no_have_val)
		Ajax.call('http://39.108.6.181/0rongkey/themes/68ecshopcom_360buy/js/user.php?act=add_book_goods', 'id=' + goods_id + '&no_have=' + no_have+ '&num=' + num+ '&tel=' + tel+ '&em=' + email, tellmeResponse, 'GET', 'JSON');
	}
}
function closeDiv1(){
