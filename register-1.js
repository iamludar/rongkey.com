...
function checkEmailExist(email, callback) {
	...
	$.post('http://www.rongkey.com/english/js/register.php?act=check_email_exist', {
		email: email
	}, function(result) {
		if (result == 'false') {
...
function checkMobilePhoneExist(mobile, callback) {
	...
	$.post('http://www.rongkey.com/english/js/register.php?act=check_mobile_exist', {
		mobile: mobile
	}, function(result) {
		if (result == 'false') {
...
function sendEmailCode(emailObj, emailCodeObj, sendButton) {
	...
	checkEmail(emailObj, function(result) {
		if (result) {
			...
			var url = 'http://www.rongkey.com/english/js/register.php?act=send_email_code';
			$.post(url, {
...
function sendMobileCode(mobileObj, mobileCodeObj, sendButton) {
	checkMobilePhone(mobileObj, function(result) {
		if (result) {
			...
			// 发送邮件
			var url = 'http://www.rongkey.com/english/js/register.php?act=send_mobile_code';
			$.post(url, {
				XDEBUG_SESSION_START: 'ECLIPSE_DBGP',
				captcha: $("#captcha").size() > 0 ? $("#captcha").val() : "",
