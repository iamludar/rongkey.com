...
//文章列表页左侧菜单
$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.sidebar-article-menu li.curr').parents('ul').addClass('curr').parents('.sidebar-article-menu').addClass('curr');
//频道页左侧菜单
...
$('.category-wrap .slide-aside').click(function(){
	if($('.category-wrap .aside').width() == 0){
		...
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').removeClass('main1210').animate({'padding-left':225},500);
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').find('.item:nth-child(4n)').addClass('last');
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').find('.item:nth-child(5n)').removeClass('last');
	}else{
		$('.category-wrap .aside').animate({'width':0},500);
		$('.category-wrap .aside-inner').animate({'width':0},500,function(){
			$(this).hide();	
		});
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').addClass('main1210').animate({'padding-left':0},500);
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').find('.item:nth-child(4n)').removeClass('last');
		$('http://www.rongkey.com/themes/68ecshopcom_360buy/js/.category-wrap .main').find('.item:nth-child(5n)').addClass('last');
	}
	if($('.main').hasClass('main1210')){
		$('#compareBox').css({'left':Math.ceil(($(window).width()-990)/2)})	
	}else{
